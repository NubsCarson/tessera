...
| 9 | **Shared-key boundary (issuer ↔ exit)** — convergent single-winner bootstrap so issuer (mints) + exit (verifies) share one ARC key via `TESSERA_KEY_FILE` | ✅ built + tested | `crates/tessera-issuer/src/keyfile.rs` (`ensure_shared_key`); re-exported/used in `crates/tessera-proxy/src/main.rs`; doc [`docs/DEPLOYMENT_TOPOLOGY.md`](./DEPLOYMENT_TOPOLOGY.md) §3 |
...
| 13 | **TEE deploy path (dstack / Intel TDX)** — verifiable non-logging relay; Docker network compose (issuer+relay+exit+client) + a TEE compose | 🔧 optional / partial — Docker network is built + E2E-tested; the TEE compose deliberately wires only relay+exit and **defers issuer/client + KMS-sealed key bootstrap** (documented as the next step, **not done**) | `deploy/docker-compose.yaml`, `deploy/dstack/docker-compose.yaml`, `Dockerfile`; docs [`docs/DEPLOY.md`](./DEPLOY.md) §2, [`docs/DEPLOYMENT_TOPOLOGY.md`](./DEPLOYMENT_TOPOLOGY.md) §3 |
...
| 17 | **Deployed clean-IP exit + Tor/Nym anonymity crowd + third-party audit + multi-party MPC ceremony** | 🔒 external — never simulated; the gaps between the runnable artifact and a stranger safely using it | tracked in [`docs/CEILING_PROGRESS.md`](./CEILING_PROGRESS.md) (E1–E16) |
...
- **TEE status nuance.** README §"Run it yourself" presents the dstack TEE as the
  ...
  exit** and **defers** the issuer/client and the KMS-sealed key bootstrap (row
  13 reflects this — *partial*, not a turnkey full-network TEE deploy).

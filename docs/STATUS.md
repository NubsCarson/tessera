...
| 13 | **TEE deploy path (dstack / Intel TDX)** — verifiable non-logging relay; Docker network compose (issuer+relay+exit+client) + a TEE compose; key-provider interface parses `ephemeral|file|dstack-kms` | 🔧 optional / partial — Docker network is built + E2E-tested; the TEE compose deliberately wires only relay+exit; `dstack-kms` is reserved and fails closed until a real KMS client is implemented | `deploy/docker-compose.yaml`, `deploy/dstack/docker-compose.yaml`, `Dockerfile`; `crates/tessera-issuer/src/key_provider.rs`; docs [`docs/DEPLOY.md`](./DEPLOY.md) §2, [`docs/DEPLOYMENT_TOPOLOGY.md`](./DEPLOYMENT_TOPOLOGY.md) §3 |

...
| **Clean onion egress lane** — client→exit over `.onion` ([`docs/CLEAN_ONION_EGRESS.md`](./docs/CLEAN_ONION_EGRESS.md)) | ✅ single-hop onion route (relay bypassed; exit's peer is the Tor circuit, never the client IP), cold-start retry + self-skip when Tor down, onion-aware signed directory v2; **a genuinely clean egress IP + a real Tor crowd stay external** |
